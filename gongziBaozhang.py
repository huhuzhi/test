...
def getgongziBaozhang():
    ...
    r1 = s.post("http://portal.sx.cmcc/pkmslogin.form", headers=headers, data=payload)
    print ("登录中.",end=""),
#    print(r1.text)
    ...
    print(".",end="")
    r6 = s.get("http://portal.sx.cmcc/sxmcc_wcm/portalchannelexample/daiban/daibai_new.jsp")
    print(".",end="")
    r7 = s.get("http://portal.sx.cmcc/sxmcc_was/uploadResource/clientinforecord/getclientinfo.jsp?firstLogin=null")
    print(".",end="")
...
#    r9 = s.get("http://portal.sx.cmcc/sxmcc_wcm/middelwebpage/app_recoder_log.jsp?app_flg=eFinance")
    r9 = s.get("http://portal.sx.cmcc/pend1/pend/dbzq/workitem/app_recoder_log.jsp?app_flg=fmbz")
    print(".",end="")
#    print r9.text
...
#    print(resp.text)
    ...
    claimData=re.findall('Name="claimId.*?>(.*?)</Field>.*?applyDate.*?>(.*?)</Field>.*?processState.*?>(.*?)</Field>', resp.text, re.S|re.M)
#    print claimData
    claimId=''
...
#    print detailURL
    ...
    tableData = re.findall(u'/>审批意见</td>.*?<TABLE.*?>(.*?)</table>', detailResp.text.lower(), re.S|re.M|re.IGNORECASE)
#    print tableData[0]
    try:
    ...
    #        print item
            ...
            print (data[0],'\t',data[1],'\t',data[2],'\t',data[3],'\t',data[5],'\t',data[6].strip(),'\t',data[7],'\t',data[8],'\n',end=""),
    except:
        print("NULL")
